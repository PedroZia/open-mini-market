...
/**
 * Concorrência do último ADMIN na API (passo 1008) contra PostgreSQL real (Dev Services): dois
 * disables simultâneos de ADMINs distintos não podem zerar os ADMINs.
 *
 * <p>O lock pessimista nos ADMINs ativos (<code>RoleStore.lockActiveUserIdsWithRole</code>)
 * serializa as duas transações: quem perde a corrida reavalia a lista já sem o ADMIN desativado
 * pelo vencedor, vê que sobrou um e recebe o 409 do passo 112 — sem retry e sem mudança de
 * contrato.
 *
 ...
 * invariante no banco (sobra exatamente um ADMIN ativo) e a <em>contagem de efeitos</em> (um 200,
 * um 409, um {@code USER_DISABLED}), nunca a ordem em que as threads chegaram.
 *
 * <p>O request HTTP comita, então o {@code @AfterEach} apaga o que o teste criou na ordem das FKs
 * ({@code user_roles} antes de {@code users}); os eventos de auditoria saem junto com o ADMIN da
 * fixture, no {@link IntegrationTestBase}.
 */
@QuarkusTest
class DisableUserConcurrencyResourceTest extends IntegrationTestBase {
  ...
  @Test
  @DisplayName(
      "dois disables simultâneos de ADMINs distintos: um 200 e um 409, e sobra exatamente um ADMIN ativo")
  void disablesOnlyOneAdminUnderRace() throws Exception {
    String token = adminToken();
    String firstAdmin = createAdmin("admin.um." + SUFFIX, "Admin Um");
    String secondAdmin = createAdmin("admin.dois." + SUFFIX, "Admin Dois");
    // O ADMIN da fixture sai da contagem, como no teste sequencial do 112: sem isso ele seria o
    // ADMIN que "sobra" e os dois disables passariam. O soft delete não derruba a sessão de quem
    // faz as chamadas.
    softDeleteUser(TestAdmin.USERNAME);

    List<Response> responses =
        race(() -> disable(firstAdmin, token), () -> disable(secondAdmin, token));

    assertThat(statuses(responses))
        .as("o lock serializa: um desativa e o outro recebe o conflito do último ADMIN")
        .containsExactlyInAnyOrder(200, 409);
    assertThat(withStatus(responses, 200).jsonPath().getString("status")).isEqualTo("DISABLED");
    ...
    // Invariantes finais: o perdedor continua ativo, sobra exatamente um ADMIN e um único evento.
    assertThat(List.of(statusOf(firstAdmin), statusOf(secondAdmin)))
        .as("exatamente um dos dois foi desativado")
        .containsExactlyInAnyOrder("ACTIVE", "DISABLED");
    assertThat(countActiveAdmins()).as("nunca zera os ADMINs").isEqualTo(1);
    assertThat(countUserDisabledEvents(firstAdmin, secondAdmin))
        .as("um único USER_DISABLED, o do vencedor")
        .isEqualTo(1);
  }
  ...
  /** POST /disable sem status esperado: o status é o resultado da corrida. */
  private static Response disable(String id, String token) {
    return given()
  ...
  /** Status persistido do usuário, lido direto do banco. */
  private String statusOf(String id) throws SQLException {
    try (Connection connection = dataSource.getConnection();
        PreparedStatement statement =
            connection.prepareStatement("select status from users where id = ?::uuid")) {
      statement.setString(1, id);
      try (ResultSet resultSet = statement.executeQuery()) {
        assertThat(resultSet.next()).as("usuário %s gravado", id).isTrue();
  ...
  /** Eventos de desativação dos dois alvos: um por desativação efetivada. */
  private int countUserDisabledEvents(String firstId, String secondId) throws SQLException {
    return queryInt(
        "select count(*) from audit_events where action = 'USER_DISABLED'"
  ...
  /**
   * O request HTTP comita, então os usuários criados aqui são removidos ao fim do teste: os testes
   * de repositório assumem a tabela como a encontraram. A FK de {@code user_roles} é {@code on
   * delete restrict}, por isso as linhas de papel saem antes.
   */
  @AfterEach
